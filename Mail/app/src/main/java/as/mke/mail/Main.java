...
//        props.put("mail.pop3.auth", "true");
        ...
        try {
            ...
            for (int i = 0; i < length; i++) {
				...
				Multipart multipart=null;// = (Multipart)messages[i].getContent();
				DataSource source = new ByteArrayDataSource(messages[i].getInputStream(), "multipart/*");
				if(messages[i].isMimeType("multipart/*")) {
					multipart = new MimeMultipart(source);
				...
				for(int ii=0; ii<count; ii++) {
					...
					if(type.equals("multipart/alternative")) {        // HTML （文本和超文本组合）
						...
						byte[] bts = new byte[n];
						n=hs.read(bts,0,n);
						String textBody = new String(bts,0,n, "UTF-8");
						System.out.println("纯文本:" +textBody);
						...
						byte[] bts = new byte[n];
						n=hs.read(bts,0,n);
						String htmlBody = new String(bts,0,n, "UTF-8");
					
						System.out.println("HTML元素:" +htmlBody);
						//part.getContent().toString());
